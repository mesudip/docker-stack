...
def test_when_create_object_with_old_data___then_reuse_old_object():
    ...
    command=config_manager.update("pytest_config", "sudip")
    assert command[0] == 'pytest_config'
    command[1].execute()
    
     # Create secret and assert
    command=secret_manager.update("pytest_secret", "sudip")
    assert command[0] == 'pytest_secret'
    command[1].execute()
    
    command=config_manager.update("pytest_config", "sudip1")
    assert command[0] == 'pytest_config_v2'
    command[1].execute()
    
     # Create secret and assert
    command=secret_manager.update("pytest_secret", "sudip1")
