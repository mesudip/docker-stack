...
class DockerObjectManager:
    ...
    def create(self, object_name, object_content, labels=""):
        ...
        return Command(command, stdin=object_content)
    ...
    def update(self, object_name, object_content) -> Tuple[str,Command]:
        sha_hash = self.calculate_hash(object_name, object_content)
        
        # Check if any version of the object already exists by its label
        command = ["docker", self.object_type, "ls", "--filter", f"label=mesudip.object.name={object_name}", "--format", "{{json .}}"]
        ...
        for object_info in existing_versions.values():
            object_name_in_docker = object_info["Name"]
            command = ["docker", self.object_type, "inspect", object_name_in_docker, "--format", "{{json .Spec.Labels}}"]
            inspect_result = run_cli_command(command, raise_error=True, log=self.log)
            labels_info = json.loads(inspect_result)
            object_sha_hash = labels_info.get("sha256")
        ...
        labels = f"mesudip.object.version={new_version:01d},mesudip.object.name={object_name}"
        return new_object_name, self.create(new_object_name, object_content, labels)
